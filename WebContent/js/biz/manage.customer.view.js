...
com.myAmway.Customer = {
	...
	queryUrl : "/myAmway/manage/customer/getCustomerList",
	
	getQueryCriteriaString : function() {
		var texts = $("#searchCriteria input[type='text']");
		var checkboxes = $("#searchCriteria input[type='checkbox']");
		var queryCriteria = new Array();
		$.each(texts, function(k, textObj){
			var key = $(textObj).attr("name");
			var val = $.trim($(textObj).val());
			if(val.length==0) val = "";
			var obj = [{key : val}];
			console.log(obj);
			queryCriteria.push(obj);
		});
		$.each(checkboxes, function(k, textObj){
			var key = $(textObj).attr("name");
			var val = $.trim($(textObj).attr("checked"));
			console.log(val);
//			queryCriteria.push(obj);
		});
	},
	...
	init : function() {
		this.initBtns();
		var self = this;
		this.getQueryCriteriaString();
		this.customerListGrid = $("#customerList").jqGrid(
			{
			url : com.myAmway.Customer.queryUrl,
			datatype : "json",
...
//		$("#mysearch").filterGrid("#customerList",{
//			"groupOp":"AND",
//			"rules":[{"field":"mobile","op":"cn","data":"1"},
//			         {"field":"mobile2","op":"ge","data":"2"}]});
...
//		this.bindDraggable();
	...
	bindQueryButton : function() {
		$("#queryData").bind("click", function(){
			com.myAmway.Customer.getQueryCriteriaString();
//			if(com.myAmway.Customer.customerListGrid) {
//				var url = com.myAmway.Customer.queryUrl;
//				com.myAmway.Customer.customerListGrid.jqGrid('setGridParam', { url: url }).trigger("reloadGrid");
//			}
		});
	},
